...
public class UserDashboardController extends UnicastRemoteObject implements ClientInterface {
    ...
    @FXML
    public Button refreshSubscribedChatsBtn;
    @FXML
    ...
    private void loadChats() {
        ...
        try {
            // Refresh subscriptions BEFORE UI
            updateSubscribedChats();
            ...
            for (Chat chat : chats) {
                ...
                // If user is subscribed, add to "My Subscribed Chats"
    ...
    @FXML
    void refreshSubscribedChats(ActionEvent event) {
        updateSubscribedChats();  // update list of subscribed chat IDs
        loadChats();              // reload UI
        //System.out.println("Subscribed chats refreshed");
    }
    ...
    private synchronized void updateSubscribedChats() {
        if (chatService != null && user != null) {
            try {
                ...
                List<Integer> newSubscribedChatIds = new ArrayList<>();

                for (Chat chat : chats) {
                    List<String> subscribers = chatService.getSubscribedUsers(chat.getChatId());

                    // Debug output
                    System.out.println("Chat " + chat.getChatId() + " subscribers: " + subscribers);

                    if (subscribers.contains(user.getUsername())) {
                        newSubscribedChatIds.add(chat.getChatId());
                    }
                }

                subscribedChatIds.clear();
                subscribedChatIds.addAll(newSubscribedChatIds);

                System.out.println("✔ Refreshed subscribedChatIds: " + subscribedChatIds);
            } catch (Exception e) {
                System.err.println("❌ Error while updating subscribed chats: " + e.getMessage());
                e.printStackTrace();
            }
        }
    }
...
*/
    ...
    private void openChat(Chat chat) {
        try {
            ...
            stage.setTitle(nickname + "'s Chat - " + chatName);
